...
async function walk(root, skip = skipSetFor(), rootLabel = "vault", onSkip, writable = rootLabel === "vault") {
  const out = [];
  async function rec(dir, depth) {
    ...
    for (const ent of entries) {
      ...
      if (out.length >= MAX_WALK_FILES) {
        throw new Error(
          `\u626B\u63CF\u6587\u4EF6\u6570\u8D85\u8FC7\u5B89\u5168\u4E0A\u9650 ${MAX_WALK_FILES}\uFF08\u6839\uFF1A${root}\uFF09\uFF1B\u8BF7\u7528 config.skipDirs \u6392\u9664\u65E0\u5173\u76EE\u5F55\uFF0C\u6216\u62C6\u5206 searchRoots / \u68C0\u67E5\u662F\u5426\u8BEF\u914D\u4E86\u8FC7\u5927\u7684\u6839\u76EE\u5F55`
        );
      }
      try {
...
async function walkRoots(roots, skip = skipSetFor(), onSkip) {
  const out = [];

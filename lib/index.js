...
import { basename as basename2, dirname as dirname7, join as join7, resolve as resolve4 } from "node:path";
...
function replaceNoteTitle(raw, newTitle) {
...
function noteTitleOf(raw) {
  const { meta, body } = parseFrontmatter(raw);
  if (meta?.title) return meta.title;
  const heading = /^#\s+(.+)$/m.exec(body);
  return heading ? heading[1].trim() : "";
}
function rewriteLinkLines(body, opts) {
...
function expectPathFor(vaultRoot, fileName = EXPECT_FILE) {
  const name2 = String(fileName ?? "").trim() || EXPECT_FILE;
  if (name2.includes("/") || name2.includes("\\")) {
    throw new Error(`\u7B14\u8BB0\u671F\u671B\u6587\u4EF6\u540D\u4E0D\u80FD\u542B\u8DEF\u5F84\u5206\u9694\u7B26\uFF1A${name2}\uFF08\u5E94\u53EA\u662F vault \u6839\u4E0B\u7684\u6587\u4EF6\u540D\uFF09`);
  }
  const p = resolve2(vaultRoot, name2);
  if (!withinRoot(vaultRoot, p)) throw new Error(`\u7B14\u8BB0\u671F\u671B\u8DEF\u5F84\u8D8A\u754C\uFF1A${name2}`);
  return p;
}
...
function summarizeLint(reports) {
  ...
  return {
    ...
    rules: [...ruleHits.entries()].map(([id, count]) => ({ id, title: ruleTitle(id), count })).sort((a, b) => b.count - a.count || compareText(a.id, b.id)),
    worst: reports.map((r) => ({ title: r.title, count: r.findings.length })).filter((r) => r.count > 0).sort((a, b) => b.count - a.count || compareText(a.title, b.title))
  };
}
...
// src/config.ts
var USER_CONFIG_FILENAME = "study-buddy.json";
var VAULT_ROOT_ENV = "DSH_STUDY_VAULT";
var VAULT_ROOT_ENV_ALIAS = "DSH_VAULT_ROOT";
...
function dshHome(env = process.env) {
  ...
  return fromEnv ? resolve3(fromEnv) : join3(homedir(), ".dsh");
}
function userConfigPath(env = process.env) {
  return join3(dshHome(env), USER_CONFIG_FILENAME);
}
var defaultReadFile = (path) => readFileSync(path, "utf8");
function readUserConfig(path, readFile) {
  let raw;
  try {
    raw = readFile(path);
  } catch (error) {
...
function resolveLayout(config, options = {}) {
  ...
  const merged = { ...user.config, ...config ?? {} };
  const fromEnv = vaultRootFromEnv(env);
  const rowVault = config?.vaultRoot !== void 0 && String(config.vaultRoot).trim() ? String(config.vaultRoot) : void 0;
  const fileVault = user.config.vaultRoot !== void 0 && String(user.config.vaultRoot).trim() ? String(user.config.vaultRoot) : void 0;
  const vaultRootRaw = rowVault ?? fromEnv.value ?? fileVault;
  const vaultRootFrom = rowVault !== void 0 ? "\u58F0\u660E\u884C config.vaultRoot" : fromEnv.from ?? (fileVault !== void 0 ? `\u6587\u4EF6 ${file}` : "\uFF08\u672A\u63D0\u4F9B\uFF09");
  if (vaultRootRaw === void 0) {
    throw new Error(
      `dsh-study-buddy \u9700\u8981 vault \u6839\u76EE\u5F55\uFF0C\u4E09\u79CD\u7ED9\u6CD5\u4EFB\u9009\u4E00\u79CD\uFF08\u6309\u4F18\u5148\u7EA7\uFF09\uFF1A\u2460 \u58F0\u660E\u884C presets/study.patch.yml \u7684 study \u884C config.vaultRoot\uFF1B\u2461 \u73AF\u5883\u53D8\u91CF ${VAULT_ROOT_ENV}\uFF08\u522B\u540D ${VAULT_ROOT_ENV_ALIAS}\uFF09\uFF1B\u2462 \u7528\u6237\u7EA7\u914D\u7F6E ${file} \u7684 {"vaultRoot":"<\u4F60\u7684 Obsidian vault \u7EDD\u5BF9\u8DEF\u5F84>"}\u3002\u4E09\u8005\u90FD\u6CA1\u6709 \u21D2 \u65E0\u6CD5\u6302\u8F7D\uFF08\u6545\u610F fail-loud\uFF1A\u9759\u9ED8\u7528\u9519\u76EE\u5F55\u4F1A\u628A\u7B14\u8BB0\u5199\u8FDB\u522B\u5904\uFF09\u3002`
    );
  }
  const vaultRoot = resolve3(vaultRootRaw);
  ...
  const layout = {
    ...
    lint: merged.lint,
    indexTtlMs: Number.isFinite(Number(merged.indexTtlMs)) && Number(merged.indexTtlMs) >= 0 ? Number(merged.indexTtlMs) : 2e3,
    maxWalkFiles: Number.isFinite(maxWalkFiles) && maxWalkFiles >= 1 ? Math.floor(maxWalkFiles) : MAX_WALK_FILES,
    planTtlHours: Number.isFinite(Number(merged.planTtlHours)) && Number(merged.planTtlHours) > 0 ? Number(merged.planTtlHours) : 24,
...
function planFileFor(vaultRoot, planId, stateDir = ".study") {
  ...
  const p = join4(vaultRoot, stateDir, "plans", `${id}.json`);
...
var HOST_CONTRACTS = [
  ...
  },
  {
    ...
    what: `\u6CE8\u5165\u6D88\u606F\u7684 source.kind \u5FC5\u987B\u662F\u975E\u7A7A\u5B57\u7B26\u4E32\u4E14 \u2260 'plugin'\uFF08\u751F\u4EA7\u8005\u81EA\u6709 kind\uFF1B\u7B2C\u4E09\u65B9\u63D2\u4EF6\u7528 plugin:<\u5305\u540D>\uFF0C\u89C1\u5E73\u53F0 llm/src/message.ts:103 \u7684 MessageSourceMap \u6CE8\u91CA"there is no shared catch-all plugin kind"\uFF09`,
    breaks: '\u9884\u6B65\u63D0\u9192\u5199\u76D8\u88AB encode \u62D2\u7EDD \u2192 \u6574\u8F6E\u5931\u8D25\uFF08"\u672C\u8F6E\u8FD0\u884C\u5931\u8D25 format v4 message requires a producer-owned source kind"\uFF09\uFF0C\u7528\u6237\u6D88\u606F\u4E5F\u4E0D\u843D\u76D8'
  },
  {
...
function serviceOf(ctx, name2) {
  ...
  if (typeof getter === "function") {
    try {
      const viaLoader = getter.call(loader?.services, name2);
      if (viaLoader !== void 0) return { value: viaLoader, viaDshLoader: true };
    } catch {
    }
  }
  return { value: ctx?.get?.(name2), viaDshLoader: false };
}
function resolveHost(ctx) {
  const looked = serviceOf(ctx, "tools");
  const toolsService = looked.value ?? ctx?.tools;
  const tools = typeof toolsService?.register === "function" ? toolsService : void 0;
  const promptLookup = serviceOf(ctx, "systemPrompt");
  const prompt = promptLookup.value;
  const systemPrompt = typeof prompt?.section === "function" ? prompt : void 0;
  const on = typeof ctx?.on === "function" ? ctx.on.bind(ctx) : void 0;
  return {
...
function sessionEventsOf(session) {
  try {
    ...
    if (typeof session?.snapshotEvents === "function") {
      ...
      return Array.isArray(snapshot) ? snapshot : void 0;
    }
    return void 0;
  } catch {
    return void 0;
...
function archiveDirFor(vaultRoot, fromId, stateDir = ".study") {
  ...
  const p = join5(vaultRoot, stateDir, "archive", id);
...
function sessionFileFor(vaultRoot, stateDir = ".study") {
  const p = join6(vaultRoot, stateDir, "session.json");
...
function summarizeOverview(notes, opts = {}) {
  ...
  const materials = [...materialChapters.entries()].map(([material, set]) => ({ material, chapters: [...set].sort(compareSection) })).sort((a, b) => compareText(a.material, b.material));
...
function formatMemory(state) {
  const entries = Object.entries(state.notes).filter(([key]) => !isControlKey(key)).sort(([a], [b]) => a === SUMMARY_KEY ? -1 : b === SUMMARY_KEY ? 1 : compareText(a, b));
  const lines = [];
...
var OPENER_SOURCE_KIND = "plugin:dsh-study-buddy";
var OPENER_SOURCE_SUMMARY = "\u5F00\u573A\u95E8\u7981\uFF1A\u5148\u8BFB\u8BB0\u5FC6\u4E0E\u8FDB\u5EA6";
...
function buildOpenerReminder(id = randomUUID()) {
  return {
    ...
    source: { kind: OPENER_SOURCE_KIND, form: "notice", summary: OPENER_SOURCE_SUMMARY }
  };
}
...
function buildToolDefs(store) {
  return [
    ...
    {
      name: "note_plan",
      description: '\u6587\u4EF6\u5939\u89C4\u5212\uFF1A\u628A"\u8FD9\u6B21\u8981\u5199\u54EA\u4E9B\u5757\u3001\u5404\u843D\u5230\u54EA\u4E2A\u76EE\u5F55"\u6574\u7406\u6210\u63D0\u6848\uFF0C\u4F9B\u7528\u6237\u62CD\u677F\u3002**\u63D0\u6848\u53EA\u5728\u5BF9\u8BDD\u91CC\uFF0C\u4E0D\u843D\u76D8**\u3002action=create\uFF08\u9ED8\u8BA4\uFF09\u51FA\u63D0\u6848\uFF1Aitems \u662F\u5F85\u843D\u5757\u6E05\u5355\uFF08title + path\uFF0Cpath \u4E3A vault \u5185\u76F8\u5BF9\u8DEF\u5F84\u542B\u6587\u4EF6\u540D\uFF09\uFF0CrootPath \u662F\u89C4\u5212\u6839\uFF0C\u4E4B\u540E note_write \u7684\u8DEF\u5F84\u5FC5\u987B\u843D\u5728\u5B83\u4E4B\u4E0B\u3002**\u7528\u6237\u62CD\u677F\u540E\u5FC5\u987B\u8C03 action=confirm \u786E\u8BA4**\uFF08rootPath \u4F20\u8981\u786E\u8BA4\u7684 planId\uFF09\u2014\u2014\u672A\u786E\u8BA4\u7684\u89C4\u5212\u4F1A\u88AB note_write \u62D2\u7EDD\uFF1B\u786E\u8BA4\u540E\u89C4\u5212\u6709\u6548\u671F\u4ECE\u786E\u8BA4\u65F6\u523B\u8D77\u7B97\uFF08\u9ED8\u8BA4 24 \u5C0F\u65F6\uFF09\uFF0C\u518D\u7528\u8BE5 planId \u9010\u4E2A note_write \u843D\u76D8\u3002\u7ED3\u6784\u6216\u987A\u5E8F\u8981\u6539\u5C31\u76F4\u63A5\u91CD\u65B0\u63D0\u6848\u4E00\u6B21\uFF08\u62FF\u5230\u65B0\u7684 planId\uFF09\u3002action=abandon \u653E\u5F03\u89C4\u5212\uFF08rootPath \u4F20\u8981\u653E\u5F03\u7684 planId\uFF09\u3002',
      parameters: {
        type: "object",
        properties: {
    ...
    {
      name: "note_write",
      description: "\u5199\u5165\u4E00\u4E2A\u7B14\u8BB0\u5757\u3002\u786C\u95E8\u7981\uFF08\u4F1A\u88AB\u62D2\u7EDD\u5E76\u5217\u4FEE\u590D\u6B65\u9AA4\uFF09\uFF1A\u2460 \u5FC5\u987B\u5148 note_expect_get \u8BFB\u8FC7\u300A\u7B14\u8BB0\u671F\u671B.md\u300B\u4E14\u4E4B\u540E\u6CA1\u6539\u8FC7\uFF1B\u2461 \u5FC5\u987B\u5E26**\u5DF2\u786E\u8BA4**\u7684 planId\uFF08\u672A\u786E\u8BA4\u7684\u89C4\u5212\u5148\u8C03 note_plan(action=confirm) \u786E\u8BA4\uFF09\uFF1B\u2462 path \u5FC5\u987B\u5728\u89C4\u5212\u6839\u4E4B\u4E0B\u3001\u6807\u9898\u5728\u89C4\u5212\u6E05\u5355\u91CC\u4E14\u672A\u88AB\u5199\u8FC7\u3002\u6B63\u6587\u5199\u6CD5\uFF08\u7ED3\u6784/\u8BE6\u7565/\u516C\u5F0F/\u56FE\u8868/\u4E92\u5F15\uFF09\u5B8C\u5168\u6309\u300A\u7B14\u8BB0\u671F\u671B.md\u300B\uFF0C**\u6CA1\u6709\u6A21\u677F\u4E0E\u5FC5\u586B\u5C0F\u8282\uFF0C\u5B57\u6570\u4E0D\u8BBE\u9650**\u3002dryRun=true \u53EA\u56DE\u663E\u5C06\u5199\u5165\u7684\u5185\u5BB9\uFF0C\u4E0D\u843D\u76D8\u3002\u76EE\u6807\u5DF2\u5B58\u5728\u65F6\u62A5\u9519\u2014\u2014\u6539\u5199\u7528 note_update\uFF0C\u907F\u514D\u8986\u76D6\u3002",
      parameters: {
        type: "object",
        properties: {
...
var VaultStore = class {
  ...
  lastMultiRoot = false;
  stateFile() {
    const p = join7(this.layout.vaultRoot, this.layout.stateDir, "progress.json");
    if (!withinRoot(this.layout.vaultRoot, p)) throw new Error("\u8FDB\u5EA6\u6587\u4EF6\u8DEF\u5F84\u8D8A\u754C");
    return p;
  }
  memoryFile() {
    const p = join7(this.layout.vaultRoot, this.layout.stateDir, "memory.json");
  ...
  rootsFor(sessionCwd) {
    ...
    if (this.layout.includeSessionCwd && cwd && !isFsRoot(cwd)) {
      roots.push({ path: resolve4(cwd), label: "\u5DE5\u4F5C\u76EE\u5F55", writable: this.layout.linkIntoNotes === true });
    }
...
${raw}`;
  ...
  async expectRules() {
    try {
      const raw = await fsp8.readFile(this.expectPath(), "utf8");
...
` + (hits.length > 0 ? hits.map((f) => `  ${f.message}${f.suggestion ? `
  ...
  async planRenameAction(ref, newTitle, sessionCwd) {
    ...
    const selfRewrite = rewriteLinkLines(parseFrontmatter(rewritten).body, { oldTitle, newTitle: title, targetId: card.id ?? void 0 });
    ...
    for (const other of index.all()) {
      ...
      void needles;
      const linkRewrite = rewriteLinkLines(otherBody, { oldTitle, newTitle: title, targetId: card.id ?? void 0 });
      ...
      const fresh = rewriteLinkLines(parsed.body, { oldTitle, newTitle: title, targetId: card.id ?? void 0 });
    ...
    let targetPath = null;
    if (plan.renameFile) {
      const candidate = join7(card.path.slice(0, card.path.length - card.fileName.length), `${plan.newBase}.md`);
...
\u5DF2\u5199\u5165\uFF1A${finalRel}${this.noteSkips()}`;
  ...
  async noteLibrary(action) {
    ...
    const lines = [
      ...
      "",
      signature === null ? `\u26A0 \u7B14\u8BB0\u671F\u671B\u6587\u4EF6\u4E0D\u5B58\u5728\uFF1A${this.expectFileName()}\uFF08\u5E94\u5728 vault \u6839\uFF09\u2014\u2014\u8BF7\u5148\u628A presets/study/assets/\u7B14\u8BB0\u671F\u671B.md \u590D\u5236\u5230 vault \u6839\u5E76\u6539\u6210\u4F60\u7684\u5199\u6CD5\uFF0C\u5426\u5219\u65E0\u6CD5\u5199\u5165` : session.expect?.signature === signature ? `- \u7B14\u8BB0\u671F\u671B\uFF1A\u5DF2\u8BFB\uFF08${this.expectFileName()}\uFF09` : `\u26A0 \u7B14\u8BB0\u671F\u671B\u5C1A\u672A\u8BFB\u53D6\uFF08\u6216\u5DF2\u66F4\u65B0\uFF09\uFF1A\u8BF7\u8C03\u7528 note_expect_get \u8BFB\u53D6\u540E\u518D\u5199\u5165`,
  ...
  async noteExpectGet() {
    ...
    try {
      ...
      throw new Error(
        `${this.expectFileName()} \u4E0D\u5B58\u5728\uFF08\u671F\u671B\u8DEF\u5F84\uFF1A${expectPath}\uFF09\u2014\u2014\u8BF7\u5148\u628A presets/study/assets/\u7B14\u8BB0\u671F\u671B.md \u590D\u5236\u5230 vault \u6839\u5E76\u6539\u6210\u4F60\u7684\u5199\u6CD5\u3002\u7B14\u8BB0\u5199\u6CD5\u53EA\u4ECE\u8FD9\u4EFD\u6587\u4EF6\u6765\uFF0C\u4EE3\u7801\u91CC\u6CA1\u6709\u5185\u5EFA\u6A21\u677F\u3002`
      );
    }
...
${raw}`;
  ...
  async notePlan(args) {
    ...
    if (action === "confirm") {
      ...
      const file2 = planFileFor(this.layout.vaultRoot, planId, this.layout.stateDir);
      const record2 = await readPlan(file2);
      if (!record2) throw new Error(`\u89C4\u5212\u4E0D\u5B58\u5728\u6216\u5DF2\u635F\u574F\uFF1A${planId}\u2014\u2014\u8BF7\u91CD\u65B0\u8C03\u7528 note_plan \u63D0\u6848`);
      if (record2.confirmed) {
        return `\u89C4\u5212\u5DF2\u786E\u8BA4\uFF08planId\uFF1A${planId}\uFF09\uFF0C\u76F4\u63A5 note_write \u9010\u4E2A\u843D\u76D8\u5373\u53EF\u3002`;
      }
      const ttl = this.layout.planTtlHours ?? DEFAULT_PLAN_TTL_HOURS;
      if (isPlanExpired(record2, ttl)) {
        const created = Date.parse(record2.createdAt);
        const at = Number.isFinite(created) ? new Date(created).toLocaleString("zh-CN") : record2.createdAt;
        throw new Error(
      ...
      record2.confirmedAt = now.toISOString();
      record2.createdAt = now.toISOString();
      await writePlan(file2, record2);
      const session2 = await this.sessionState();
      await writeSession(this.sessionFile(), { ...session2, activePlanId: planId });
      const remaining = record2.items.map((it) => it.title);
      return [
        `\u5DF2\u786E\u8BA4\u89C4\u5212\uFF08planId\uFF1A${planId}\uFF09`,
        `- \u89C4\u5212\u6839\uFF1A${record2.rootPath || "\uFF08vault \u6839\uFF09"}`,
...
\uFF08planId\uFF1A${record.planId}\uFF1B**\u7528\u6237\u62CD\u677F\u540E**\u5148 note_plan({ action: "confirm", rootPath: "${record.planId}" }) \u786E\u8BA4\uFF0C\u518D\u9010\u4E2A note_write \u843D\u76D8\uFF09`;
  ...
  async noteWrite(input) {
    ...
    const abs = resolve4(this.layout.vaultRoot, targetRel);
...
${restored}`;
  ...
  async noteToc(dir, opts = {}) {
    ...
    const tocPath = join7(dirPathFor(this.layout.vaultRoot, norm), TOC_FILE);
    let custom = "";
    try {
...
function apply(ctx, config) {
  ...
  console.info(
    `[dsh-study-buddy] vaultRoot \u2190 ${resolved.vaultRootFrom}\uFF1A${resolved.layout.vaultRoot}\uFF08\u5BBF\u4E3B\u9002\u914D\uFF1A${host.viaDshLoader ? "dsh-loader \u7A33\u5B9A\u5165\u53E3" : "\u76F4\u63A5 Cordis \u670D\u52A1"}\uFF09`
  );
  let store;
  try {
  ...
  if (typeof tools?.register !== "function") {
    ...
    throw new Error(
      `dsh-study-buddy: ctx.tools.register \u4E0D\u53EF\u7528\uFF0C\u65E0\u6CD5\u6CE8\u518C\u7B14\u8BB0\u5DE5\u5177\uFF08\u5BBF\u4E3B\u5951\u7EA6\u70B9 ${contract?.ref ?? "toolsRegister"}\uFF1A${contract?.what ?? "\u5DE5\u5177\u6CE8\u518C\u8868"}\uFF1B\u6539\u4E86\u5C31\u5728\u8FD9\u91CC\u5BF9\uFF1Asrc/host.ts / tools/verify-contract.mjs\uFF09`
    );
  }
  const disposers = [];
  try {

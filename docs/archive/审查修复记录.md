...
> 版本基线：v1.1.0 ｜ 状态：**历史台账（v0.9 时期，已归档，不再更新）** ｜ 读者：插件维护者、二次开发者
>
> **这是什么**：2026-09 两轮代码审查的**结论与逐项修复状态**。
> 首轮（v0.8.0 重构后）47 项 + 复审（v0.9.0 修复后）15 项的**明细报告已归档**到
> [`review/`](review/)（时间点快照：行号、路径与章节引用均保持当时状态）。
> 本文件只保留"结论 + 逐项状态 + 为什么不修"；实现口径见 [`技术文档`](../技术文档.md)，设计取舍见 [`设计文档`](../设计文档.md)。
> v1.0 之后的新问题不再记在这里：进 `需求分析.md` / `设计文档.md`（见 `docs/README.md` §三）。
...
> [`archive/review/07-复审-修复验收.md`](review/07-复审-修复验收.md)）。
...
| 报告 | 内容 | 归档理由 |
| :-- | :-- | :-- |
| [`archive/review/01-隐私与安全.md`](review/01-隐私与安全.md) | SEC-1~SEC-8：注入面、跨盘符扫描、路径清洗、隐私出境、记忆注入、已核查无风险项 | 首轮时间点快照；结论已全部回填到 §二 |
| [`archive/review/02-复杂度与可维护性.md`](review/02-复杂度与可维护性.md) | CPLX-1~CPLX-11：函数长度/圈复杂度、隐藏副作用、魔法值、死代码、文档漂移、测试盲区 | 同上 |
| [`archive/review/03-可拓展性.md`](review/03-可拓展性.md) | EXT-1~EXT-8：规则注册点、模板与 lint 双口径、领域族硬编码、上帝类、配置三份同构 | 同上 |
| [`archive/review/04-业务质量.md`](review/04-业务质量.md) | BIZ-1~BIZ-12：2 个 blocker + 6 处"报告与实际不符" + 9 项细节缺陷 | 同上 |
| [`archive/review/05-性能表现.md`](review/05-性能表现.md) | PERF-1~PERF-8：实测数据、重构前后对照、触发负载与可接受性判断 | 同上（复测数据见 07 §六） |
| [`archive/review/06-架构决策.md`](review/06-架构决策.md) | ARCH 六问：真实问题、达成度、新隐患、回退成本、更小的替代方案 | 同上（处置见 §2.7） |
| [`archive/review/07-复审-修复验收.md`](review/07-复审-修复验收.md) | 第二次审查（v0.9.0 验收）：探针证据、性能复测、15 条修复副作用（N1~N15） | 复审时间点快照；处置已回填到 §三 |

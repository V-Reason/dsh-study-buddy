...
22. 边界：超长 value（>4000 字符）被拒、空 key 被拒、未知 action 被拒。
...
- **D**：我发“归档”——生成 1~3 张 `【验证】` 卡 + `card_moc`，只重组去重、不加新知识。

## 5 · 收尾
23. 恢复进度/记忆：把第 2 步记录的原值写回（原值为空则 clear，非空则 set 回原值）。
24. 清理测试数据：列出全部创建的 `【验证】` 文件路径（卡 + MOC）；尝试用你可用且安全的工具删除；删除不了就给出手动删除清单（或提示可用 git 还原）。
25. 输出汇总报告：按 0–5 分组逐项 ✔/✘/⚠ + 一行证据；✘ 项给出原因与建议。
